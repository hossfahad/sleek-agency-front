...
const WavesShader: React.FC = () => {
  const containerRef = useRef<HTMLDivElement>(null);

  useEffect(() => {
    // Load ShaderProgram script dynamically
    const script = document.createElement('script');
    script.src = 'https://cdn.jsdelivr.net/gh/bsehovac/shader-program@1.0.0/shader-program.min.js';
    script.async = true;
  ...
  const initShader = () => {
    if (!containerRef.current || !window.ShaderProgram) return;
    
    const pointSize = 2.5;
    
    const waves = new window.ShaderProgram(containerRef.current, {
